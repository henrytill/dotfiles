...
-- Load custom C fold queries from file
local c_folds_path = vim.fn.stdpath("config") .. "/queries/c/folds.scm"
local c_folds_file = io.open(c_folds_path, "r")
if c_folds_file then
	local c_folds_content = c_folds_file:read("*all")
	c_folds_file:close()
	vim.treesitter.query.set("c", "folds", c_folds_content)
end
